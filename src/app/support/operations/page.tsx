...
import { getRecentPublicSupportInquiries } from '@/lib/support-inquiry';
...
export default async function SupportOperationsPage({
  ...
  const publicInquiries =
    inquiryResult.status === 'fulfilled' ? inquiryResult.value : [];
  const searchError =
  ...
  return (
    <PortalShell user={operator}>
      <div className="box-border flex w-full max-w-md min-w-0 flex-col gap-4">
          <header className="w-full min-w-0 rounded-3xl border border-[#D8A649]/40 bg-[#EEF5F1] p-5 shadow-sm shadow-emerald-950/5">
          ...
          <Card className="scroll-mt-28" id="public-inquiries">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Inbox className="size-5 text-[#052F26]" aria-hidden="true" />
                Public support inquiries
              </CardTitle>
              <CardDescription>
                The latest messages submitted through the public support form.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-3 pb-5 pt-4">
              {publicInquiries.length ? (
                publicInquiries.map((inquiry) => (
                  <article
                    className="min-w-0 rounded-2xl border border-emerald-950/10 bg-[#EEF5F1] p-4"
                    key={inquiry.id}
                  >
                    <div className="flex min-w-0 items-start justify-between gap-3">
                      <span className="min-w-0">
                        ...
                        <span className="mt-1 block text-xs font-medium text-slate-500">
                          {notificationDate(inquiry.createdAt)}
                        </span>
                      </span>
                    ...
                    <div className="mt-3 grid min-w-0 gap-1 text-xs font-bold text-[#052F26]">
                      <a className="min-w-0 break-all hover:underline" href={`mailto:${inquiry.email}`}>
                        {inquiry.email}
                      </a>
                      <a className="min-w-0 break-all hover:underline" dir="ltr" href={`tel:${inquiry.phone}`}>
                        {inquiry.phone}
                      </a>
                    </div>
                  </article>
                ...
                <p className="rounded-2xl border border-dashed border-emerald-200 p-4 text-sm leading-6 text-slate-500">
                  No public support inquiries are available yet.
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="scroll-mt-28" id="student-lookup">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Search className="size-5 text-[#052F26]" aria-hidden="true" />
                Find a student
              </CardTitle>
              <CardDescription>
            ...
            <CardContent className="pb-5 pt-4">
              <form action="/support/operations" className="flex min-w-0 flex-col gap-3" method="get">
          ...
          {selectedStudent ? (
            <>
              <Card>
                <CardHeader>
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#052F26] text-white">
                ...
                <CardContent className="flex flex-col gap-3 pb-5 pt-4">
                  <p className="flex min-w-0 items-start gap-3 rounded-xl bg-[#EEF5F1] p-3 text-sm text-slate-700">
                    <Mail className="mt-0.5 size-4 shrink-0 text-[#052F26]" aria-hidden="true" />
                    <span className="min-w-0 break-all">{selectedStudent.email}</span>
                  </p>
                  <p className="flex min-w-0 items-start gap-3 rounded-xl bg-[#EEF5F1] p-3 text-sm text-slate-700">
                    <Phone className="mt-0.5 size-4 shrink-0 text-[#052F26]" aria-hidden="true" />
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <BookOpen className="size-5 text-[#052F26]" aria-hidden="true" />
                ...
                <CardContent className="flex flex-col gap-3 pb-5 pt-4">
                  {selectedStudent.enrollments.length ? (
                    selectedStudent.enrollments.map((enrollment) => (
                      <div
                        className="flex min-w-0 items-start justify-between gap-3 rounded-2xl border border-emerald-950/10 bg-[#EEF5F1] p-4"
                        key={enrollment.id}
                      >
                        <span className="min-w-0">
                          <span className="block truncate text-sm font-black text-slate-900">
                ...
                <CardContent className="pb-5 pt-4">
                  <form action={resetStudentPasswordAction} className="flex min-w-0 flex-col gap-3">
                    ...
                    <Input
                      ...
                      type="password"
                    />
                    <label className="flex min-w-0 items-start gap-3 rounded-2xl border border-[#D8A649]/40 bg-[#EEF5F1] p-4 text-sm leading-6 text-[#805C1D]">
                      <input
                    ...
                    <ActionSubmitButton
                      className={cn(
                        buttonVariants(),
                        'w-full bg-[#052F26] text-white hover:bg-[#093F33]',
                      )}
              ...
              <Card className="scroll-mt-28" id="support-tickets">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Bell className="size-5 text-[#052F26]" aria-hidden="true" />
                ...
                <CardContent className="pb-5 pt-4">
                  {selectedStudent.notifications.length ? (
                    <form action={resendNotificationAction} className="flex min-w-0 flex-col gap-3">
                      ...
                      <select
                        className="h-12 w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-4 text-sm text-slate-900 outline-none focus:border-[#052F26] focus:ring-4 focus:ring-emerald-100"
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Send className="size-5 text-[#052F26]" aria-hidden="true" />
            ...
            <Card className="border-red-400/20">
              ...
              <CardContent className="pb-5 pt-4">
                <Link
                  className="inline-flex h-11 w-full items-center justify-center rounded-xl bg-[#052F26] px-4 text-sm font-black text-white transition hover:bg-[#093F33]"
