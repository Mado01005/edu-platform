  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
         <div className="relative space-y-8 overflow-hidden rounded-[3rem] border border-emerald-950/10 bg-white p-6 shadow-sm shadow-emerald-950/5 sm:p-10">
           ...
           <div className="space-y-6">
             ...
             <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button onClick={() => { if (!newTeacherEmail) return; updateRole(newTeacherEmail, 'superadmin'); setNewTeacherEmail(''); }} className="flex-1 rounded-[1.5rem] bg-[#063A2F] py-5 text-[10px] font-black uppercase tracking-widest text-white shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-[#0A4235] hover:shadow-md active:translate-y-0">Make Administrator</button>
             </div>
           </div>
         </div>
         ...
         <div className="relative space-y-8 overflow-hidden rounded-[3rem] border border-emerald-950/10 bg-white p-6 shadow-sm shadow-emerald-950/5 sm:p-10">
            ...
            <div className="h-1 w-full overflow-hidden rounded-full bg-slate-100">
               <div className="h-full w-1/3 bg-[#063A2F]"></div>
            </div>
         </div>
      </div>
      ...
      <div className="min-h-[500px] overflow-hidden rounded-[3rem] border border-emerald-950/10 bg-white shadow-sm shadow-emerald-950/5">
         <div className="relative flex items-center justify-between overflow-hidden border-b border-emerald-950/10 bg-[#F4F7F4] px-6 py-8 sm:px-10">
            <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-600">Team &amp; User Accounts</h3>
            <span className="rounded-2xl border border-emerald-200/60 bg-emerald-50 px-4 py-2 text-[11px] font-black text-[#063A2F]">{allRoles.length} Accounts</span>
         </div>
         
         <div className="divide-y divide-emerald-950/10">
           <div>
              {allRoles.filter(r => r.role === 'teacher' || r.role === 'superadmin').map(r => (
                <div key={r.email} className="group flex flex-col gap-6 border-l-4 border-transparent px-6 py-8 transition-all duration-200 ease-in-out hover:border-[#063A2F] hover:bg-emerald-50 sm:px-10 md:flex-row md:items-center md:justify-between">
                   <div className="flex min-w-0 items-center gap-4 sm:gap-8">
                      <div className={`flex size-16 items-center justify-center rounded-2xl border text-xl font-black shadow-sm transition-all duration-200 ${r.role === 'superadmin' ? 'border-[#063A2F] bg-[#063A2F] text-white' : 'border-emerald-950/10 bg-slate-100 text-slate-600'}`}>
                        {accountLabel(r.email).charAt(0).toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="break-all text-xl font-black tracking-tighter text-slate-900 transition-colors group-hover:text-[#063A2F]">{accountLabel(r.email)}</p>
                        <p className={`mt-2 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] ${r.role === 'superadmin' ? 'border-[#D8A84E]/40 bg-[#F4F7F4] text-[#805C1D]' : 'border-emerald-200 bg-emerald-50 text-[#063A2F]'}`}>
                          <span className={`size-2 rounded-full ${r.role === 'superadmin' ? 'bg-[#D8A84E]' : 'bg-[#063A2F]'}`}></span>
                          {r.role === 'superadmin' ? 'ADMINISTRATOR' : 'TEACHER'}
                        </p>
                      </div>
                   </div>
                   <div className="flex w-full flex-col gap-3 sm:flex-row md:w-auto md:items-center">
                     <button onClick={() => setSelectedUser(r)} className="rounded-2xl border border-emerald-950/10 bg-white p-4 text-slate-500 opacity-0 shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-emerald-300 hover:text-[#063A2F] hover:shadow-md group-hover:opacity-100">
                     ...
                     <div className="flex w-full flex-col gap-2 md:min-w-[200px]">
                         {r.role !== 'superadmin' && (
                           <button onClick={() => updateRole(r.email, 'superadmin')} className="w-full rounded-xl bg-[#063A2F] px-6 py-3 text-[9px] font-black uppercase tracking-widest text-white shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-[#0A4235] hover:shadow-md">Make Administrator</button>
                         )}
                 ...
                 {allRoles.filter(r => r.role === 'student' || r.role === 'banned').map(r => (
                   <div key={r.email} className={`card-hover group/student flex flex-col items-center justify-between gap-8 overflow-hidden rounded-[2rem] border bg-white p-6 shadow-sm shadow-emerald-950/5 md:flex-row ${r.role === 'banned' ? 'border-red-200/80 grayscale' : 'border-emerald-950/10'}`}>
                      <div className="flex w-full min-w-0 items-center gap-4 sm:gap-8 md:w-auto">
                         <div className={`flex size-16 items-center justify-center rounded-[1.5rem] text-xl font-black transition-all duration-200 ${r.role === 'banned' ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-[#063A2F] group-hover/student:bg-[#063A2F] group-hover/student:text-white'}`}>{accountLabel(r.email).charAt(0).toUpperCase()}</div>
                         <div className="min-w-0">
                            ...
                            <p className={`mt-2 inline-flex rounded-full border px-3 py-1 text-[9px] font-black uppercase tracking-[0.2em] ${r.role === 'banned' ? 'border-red-200 bg-red-50 text-red-600' : 'border-emerald-200 bg-emerald-50 text-[#063A2F]'}`}>{r.role === 'banned' ? '✘ Access disabled' : 'STUDENT · Active account'}</p>
                         </div>
                      </div>
                      <div className="flex w-full flex-col gap-3 sm:flex-row md:w-auto md:items-center">
                        <button onClick={() => setSelectedUser(r)} className="rounded-2xl border border-emerald-950/10 bg-white p-4 text-slate-500 opacity-0 shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-emerald-300 hover:text-[#063A2F] hover:shadow-md group-hover/student:opacity-100">
                          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        </button>
                        <div className="flex w-full flex-col gap-2 md:min-w-[200px] md:w-auto">
                           {r.role !== 'banned' ? (
                             <>
                               ...
                               <button onClick={() => updateRole(r.email, 'superadmin')} className="w-full rounded-xl bg-[#063A2F] px-8 py-3 text-[9px] font-black uppercase tracking-widest text-white shadow-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:bg-[#0A4235] hover:shadow-md active:translate-y-0">Make Administrator</button>
      ...
      >
        <DialogContent>
          ...
          <p className="break-all rounded-xl border border-emerald-950/10 bg-[#F4F7F4] p-3 text-sm text-slate-700">
            {deleteTarget?.email}
          </p>
          <DialogFooter>
            <DialogClose asChild>
