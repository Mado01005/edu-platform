      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 items-start">
         <div className="xl:col-span-4 space-y-6">
            <div className="space-y-6 rounded-3xl border border-emerald-950/10 bg-white p-6 shadow-sm shadow-emerald-950/5 sm:p-8">
               <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#063A2F]">1. Select Subject and Lesson</label>
               <div className="space-y-4">
                  <select className="w-full cursor-pointer rounded-2xl border border-emerald-950/10 bg-white px-6 py-4 text-sm text-slate-900 outline-none transition focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100" value={selectedSubjectId} onChange={(e) => { setSelectedSubjectId(e.target.value); setSelectedLessonId(''); }}>
                    <option value="">Select Subject / Course</option>
                    {localSubjects.map(s => <option key={s.id} value={s.id!}>{s.icon} {s.title}</option>)}
                  </select>
                  {selectedSubjectId && (
                    <select className="w-full animate-in rounded-2xl border border-emerald-950/10 bg-white px-6 py-4 text-sm text-slate-900 outline-none transition focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100 slide-in-from-top-2" value={selectedLessonId} onChange={(e) => setSelectedLessonId(e.target.value)}>
                      <option value="">Select Lesson</option>
                      {activeLessons.map((l: LessonMeta) => <option key={l.id} value={l.id}>{l.title}</option>)}
                    </select>
                  )}
                  <div className="flex gap-2 pt-2">
                    <button type="button" onClick={handleCreateSubject} className="flex-1 rounded-xl border border-emerald-950/10 bg-[#F4F7F4] py-3 text-[9px] font-black uppercase tracking-widest text-slate-700 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-[#063A2F]">+ Create Subject</button>
                    {selectedSubjectId && <button type="button" onClick={() => handleCreateLesson(selectedSubjectId)} className="flex-1 rounded-xl border border-emerald-950/10 bg-[#F4F7F4] py-3 text-[9px] font-black uppercase tracking-widest text-slate-700 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-[#063A2F]">+ Create Lesson</button>}
                  </div>
               </div>
            </div>
         </div>
           ...
           <div className="space-y-8 animate-in slide-in-from-right-4 duration-300 xl:col-span-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <select 
                    className="w-full cursor-pointer rounded-2xl border border-emerald-950/10 bg-white px-6 py-4 text-sm text-slate-900 outline-none transition focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100"
