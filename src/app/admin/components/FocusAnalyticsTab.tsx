  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="size-8 animate-spin rounded-full border-2 border-[#052F26] border-t-transparent" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 fade-in">
      <div className="space-y-2 max-w-2xl text-center md:text-left mx-auto md:mx-0">
        ...
        <p className="text-sm font-bold uppercase leading-relaxed tracking-widest text-[#052F26]">Focus Overview</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-[2.5rem] border border-emerald-950/10 bg-white p-8 shadow-sm shadow-emerald-950/5">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-3xl font-bold text-[#052F26]">%</span>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-[2.5rem] border border-emerald-950/10 bg-white shadow-sm shadow-emerald-950/5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-emerald-950/10 bg-[#EEF5F1] text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
              ...
              {(() => {
                try {
                  return data.frictionList.map((item, idx) => {
                    ...
                    return (
                      <tr key={item.lesson_id} className="transition-colors hover:bg-[#EEF5F1]">
                    ...
                    <tr key={`${item.user_id}_${item.lesson_id}`} className="hover:bg-red-500/5 transition-colors">
                      ...
                      <td className="p-6">
                        <div className="flex items-center gap-2 text-xs font-bold tabular-nums">
                          ...
                          <span className="text-[#052F26]">{item.global_average}m</span>
                        </div>
                      </td>
                    </tr>
                  ));
