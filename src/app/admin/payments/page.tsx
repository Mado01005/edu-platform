...
export default async function AdminPaymentsPage() {
  ...
  return (
    <PortalShell user={user}>
      <header className="rounded-3xl border border-[#D8A649]/40 bg-[#EEF5F1] p-5 sm:p-7">
        <span className="flex size-11 items-center justify-center rounded-2xl bg-[#052F26] text-white"><BadgeCheck className="size-5" /></span>
      ...
      <div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 font-black"><ReceiptText className="size-4 text-[#052F26]" /> Pending transfers</p><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">{payments.length}</span></div>
