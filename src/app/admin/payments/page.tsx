...
export default async function AdminPaymentsPage() {
  ...
  return (
    <PortalShell user={user}>
      <header className="rounded-3xl border border-[#D8A84E]/40 bg-[#F4F7F4] p-5 sm:p-7">
        <span className="flex size-11 items-center justify-center rounded-2xl bg-[#063A2F] text-white"><BadgeCheck className="size-5" /></span>
        <p className="mt-5 flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-[#805C1D]"><Clock3 className="size-4" /> 1-click manual review</p>
      ...
      <div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 font-black"><ReceiptText className="size-4 text-[#063A2F]" /> Pending transfers</p><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">{payments.length}</span></div>
