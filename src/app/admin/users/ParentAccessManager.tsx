  ...
  return (
    <section className="rounded-3xl border border-emerald-950/10 bg-white p-4 shadow-sm shadow-emerald-950/5">
      ...
      <form className="mt-4 flex min-w-0 flex-col gap-2" onSubmit={configure}>
        <select className="w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-sm text-slate-900 outline-none focus:border-[#052F26] focus:ring-4 focus:ring-emerald-100" onChange={(event) => setParentId(event.target.value)} required value={parentId}><option value="">Select parent</option>{parents.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select>
        <select className="w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-sm text-slate-900 outline-none focus:border-[#052F26] focus:ring-4 focus:ring-emerald-100" onChange={(event) => setStudentId(event.target.value)} required value={studentId}><option value="">Select student</option>{students.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select>
        <input aria-label="4-digit parent PIN" className="w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-center text-lg text-slate-900 outline-none placeholder:text-slate-400 focus:border-[#052F26] focus:ring-4 focus:ring-emerald-100" inputMode="numeric" maxLength={4} onChange={(event) => setPin(event.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="4-digit PIN" required value={pin} />
