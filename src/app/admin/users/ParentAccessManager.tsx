...
export function ParentAccessManager({ links, parents, students }: { links: LinkRecord[]; parents: Option[]; students: Option[] }) {
  ...
  return (
    <section className="rounded-3xl border border-emerald-950/10 bg-white p-4 shadow-sm shadow-emerald-950/5">
      <span className="flex size-10 items-center justify-center rounded-2xl bg-amber-50 text-amber-600"><ShieldCheck className="size-5" /></span>
      ...
      <form className="mt-4 flex min-w-0 flex-col gap-2" onSubmit={configure}>
        <select className="w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-sm text-slate-900 outline-none focus:border-[#084B2B] focus:ring-4 focus:ring-emerald-100" onChange={(event) => setParentId(event.target.value)} required value={parentId}><option value="">Select parent</option>{parents.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select>
        <select className="w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-sm text-slate-900 outline-none focus:border-[#084B2B] focus:ring-4 focus:ring-emerald-100" onChange={(event) => setStudentId(event.target.value)} required value={studentId}><option value="">Select student</option>{students.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select>
        <input aria-label="4-digit parent PIN" className="w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-center text-lg text-slate-900 outline-none placeholder:text-slate-400 focus:border-[#084B2B] focus:ring-4 focus:ring-emerald-100" inputMode="numeric" maxLength={4} onChange={(event) => setPin(event.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="4-digit PIN" required value={pin} />
        <button className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber-300 px-4 py-3 text-sm font-black text-black disabled:opacity-50" disabled={pending === 'save' || pin.length !== 4} type="submit">{pending === 'save' ? <Loader2 className="size-4 animate-spin" /> : <Link2 className="size-4" />} Link and issue PIN</button>
      </form>
      {message ? <p aria-live="polite" className="mt-3 text-xs text-slate-700">{message}</p> : null}
      <div className="mt-4 flex flex-col gap-2">{links.map((link) => <div className="flex min-w-0 items-center gap-2 rounded-xl border border-emerald-950/10 bg-[#F8FAF7] p-3" key={`${link.parentId}:${link.studentId}`}><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-slate-900">{link.parentName}</span><span className="block truncate text-xs text-slate-600">Student: {link.studentName}</span></span><button aria-label="Remove parent link" className="rounded-lg border border-red-200 bg-white p-2 text-red-700" disabled={Boolean(pending)} onClick={() => void remove(link)} type="button"><Trash2 className="size-4" /></button></div>)}</div>
    </section>
  );
}
