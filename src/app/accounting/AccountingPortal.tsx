...
const fieldClass =
  'h-12 w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-4 text-sm text-slate-900 outline-none transition focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100 disabled:bg-slate-100 disabled:text-slate-500';
    ...
    <div className="flex w-full min-w-0 flex-col gap-4">
        ...
        <CardContent className="pt-5">
          <form className="flex min-w-0 flex-col gap-3" onSubmit={createPayment}>
            ...
            <textarea
              ...
              className="min-h-24 w-full min-w-0 resize-y rounded-xl border border-emerald-950/10 bg-white px-4 py-3 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100"
              maxLength={1000}
              name="notes"
              placeholder="Internal notes (optional)"
            />
            <label className="flex min-w-0 items-start gap-3 rounded-xl border border-emerald-950/10 bg-[#F4F7F4] p-3 text-sm text-slate-900">
      ...
      <Card className="scroll-mt-28" id="payment-approvals">
        ...
        <CardContent className="flex min-w-0 flex-col gap-3 pt-5">
          {onlinePayments.map((payment) => (
            <article className="rounded-2xl border border-amber-200 bg-amber-50 p-4" key={payment.id}>
              ...
              <div className="mt-3 grid min-w-0 grid-cols-3 gap-2">
                <a className="flex min-w-0 items-center justify-center rounded-xl border border-slate-300 bg-white px-2 py-2 text-xs font-black text-slate-700 hover:bg-[#F4F7F4]" href={`/api/accounting/online-payments/${payment.id}/receipt`} rel="noopener noreferrer" target="_blank">View</a>
                <button className="rounded-xl bg-[#063A2F] px-2 py-2 text-xs font-black text-white transition hover:bg-[#0A4235] disabled:opacity-50" disabled={isBusy} onClick={() => void reviewOnlinePayment(payment.id, 'approve')} type="button">Approve</button>
          ...
          {!onlinePayments.length ? <p className="rounded-2xl border border-dashed border-slate-300 bg-[#F4F7F4] p-6 text-center text-sm text-slate-600">No pending online receipts.</p> : null}
        </CardContent>
      </Card>

      <Card>
        <CardHeader><span className="flex size-11 items-center justify-center rounded-2xl bg-emerald-50 text-[#063A2F]"><Settings2 className="size-5" /></span><CardTitle className="mt-2 text-xl">Digital payment channels</CardTitle></CardHeader>
        <CardContent className="flex min-w-0 flex-col gap-3 pt-5">
          {paymentChannels.map((channel) => (
            <form className="flex min-w-0 flex-col gap-2 rounded-2xl border border-emerald-950/10 bg-[#F4F7F4] p-3" key={channel.method} onSubmit={(event) => void saveChannel(event, channel.method)}>
              ...
              <textarea className="min-h-20 w-full min-w-0 rounded-xl border border-emerald-950/10 bg-white px-4 py-3 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100" defaultValue={channel.instructions ?? ''} name="instructions" placeholder="Student instructions" />
      ...
      <Card>
        ...
        <CardContent className="flex min-w-0 flex-col gap-3 pt-5">
          ...
          {!pendingSubscriptions.length ? (
            <p className="rounded-2xl border border-dashed border-slate-300 bg-[#F4F7F4] p-6 text-center text-sm text-slate-600">
      ...
      <section className="flex min-w-0 scroll-mt-28 flex-col gap-3" id="revenue-ledger">
        ...
        {ledger.map((payment) => (
          ...
          >
            <div className="flex min-w-0 items-start gap-3">
              <FileText className="mt-0.5 size-5 shrink-0 text-[#063A2F]" />
              <span className="min-w-0 flex-1">
                <span className="block truncate font-black">
                  {payment.studentName}
                </span>
            ...
            <div className="flex min-w-0 gap-2">
              {payment.status === 'PENDING' ? (
                ...
                <Link
                  className="flex h-9 flex-1 items-center justify-center gap-2 rounded-xl bg-[#063A2F] px-3 text-sm font-black text-white transition hover:bg-[#0A4235]"
