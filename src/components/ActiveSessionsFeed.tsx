...
export default function ActiveSessionsFeed() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Anti-Piracy Matrix Alert Display */}
      {piracyAlerts.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-5 shadow-[0_0_20px_rgba(239,68,68,0.1)] mb-6 animate-pulse">
           <h3 className="text-red-400 font-bold mb-2 flex items-center gap-2">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
              CRITICAL: Account Sharing Detected
           </h3>
           <ul className="text-sm text-red-300 space-y-1">
              {piracyAlerts.map(alert => (
                 <li key={alert.email}>
                   <strong>{alert.email}</strong> is currently logged in and active from <strong>{alert.ips.length}</strong> different geographical IPs simultaneously: {alert.ips.join(', ')}
                 </li>
              ))}
           </ul>
        </div>
