  ...
  return (
    <div className="space-y-6 fade-in">
      <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.8)]"></span>
            Active Sessions
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {sessions.length === 0 && (
          <div className="col-span-full py-10 text-center text-slate-600 italic bg-[#F8FAF7] rounded-xl border border-emerald-950/10">No users connected.</div>
        )}
          ...
          <div key={session.id} className="relative bg-white border border-emerald-950/10 rounded-2xl p-5 shadow-sm hover:border-emerald-300 transition duration-300 overflow-hidden group">
            ...
            <div className="flex justify-between items-start mb-4">
               ...
               {session.is_idle ? (
                 ...
                 <span className="flex items-center gap-1.5 px-2 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-black tracking-widest uppercase rounded">
            ...
            <div className="space-y-3 pt-4 border-t border-white/5">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-xs text-emerald-200 truncate" title={session.current_page}>{session.current_page}</span>
              </div>
              <div className="flex items-center gap-2">
