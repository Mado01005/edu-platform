...
export function AssignmentSubmissionCard({
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-[#D8A84E]/40 bg-[#F4F7F4] p-4 text-slate-900">
      ...
      {submission ? (
        <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm">
          ...
          {urls.length ? <div className="mt-2 flex flex-wrap gap-2">{urls.map((url, index) => <a className="rounded-lg border border-emerald-200 bg-white px-2 py-1 text-xs font-bold text-[#063A2F]" href={url} key={url} rel="noopener noreferrer" target="_blank">Attachment {index + 1}</a>)}</div> : null}
        </div>
      ) : null}

      <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
        <label className="flex min-h-24 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-emerald-300 bg-white px-4 py-4 text-center text-sm font-black text-[#063A2F] hover:border-[#063A2F] hover:bg-emerald-50">
        ...
        <ul className="mt-3 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2">
          {files.map((file, index) => (
            <li className="flex min-w-0 items-center gap-2 rounded-lg border border-emerald-950/10 bg-white p-2 text-xs" key={`${file.name}-${file.lastModified}`}>
              <FileText className="size-4 shrink-0 text-[#063A2F]" />
      ...
      <label className="mt-3 block text-xs font-bold text-slate-700">Written solution (optional)
        <textarea className="mt-2 min-h-32 w-full min-w-0 resize-y rounded-xl border border-emerald-950/10 bg-white px-3 py-3 text-sm font-medium text-slate-900 outline-none focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100" disabled={pending} maxLength={20_000} onChange={(event) => setTextSolution(event.target.value)} placeholder="Type your reasoning or final answer here…" value={textSolution} />
      </label>
      <button className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#063A2F] px-4 text-sm font-black text-white hover:bg-[#0A4235] disabled:opacity-50" disabled={pending || (!files.length && !textSolution.trim())} onClick={() => void submit()} type="button">
