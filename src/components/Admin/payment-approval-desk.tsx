        ...
        return (
          <article className="grid min-w-0 grid-cols-1 gap-3 rounded-2xl border border-emerald-950/10 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center" key={payment.id}>
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm font-black text-[#052F26]">{payment.amount} {payment.currency} · {payment.method.replaceAll('_', ' ')}</p>
            </div>
            <div className="grid min-w-0 grid-cols-3 gap-2">
              <button className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl border border-emerald-950/10 px-3 text-xs font-black text-slate-700 hover:bg-[#EEF5F1]" onClick={() => setPreview(payment)} type="button"><ReceiptText className="size-4" /> Receipt</button>
              <button className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-[#052F26] px-3 text-xs font-black text-white hover:bg-[#093F33] disabled:opacity-50" disabled={busy} onClick={() => void review(payment, 'approve')} type="button">{pending === `${payment.id}:approve` ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} Approve</button>
