...
export function WorkspaceActionHub({
  ...
  const actions = [
    {
      accent: 'bg-brand-gold/15 text-brand-base',
  ...
  return (
    <section className="w-full min-w-0 rounded-2xl border border-brand-rim bg-white p-5 shadow-sm shadow-brand-base/5 sm:p-7">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-base">
      ...
      <div className="mt-6 grid min-w-0 grid-cols-1 gap-3 lg:grid-cols-3">
        {actions.map(({ accent, description, href, icon: Icon, label }) => (
          <Link
            className="group flex min-w-0 flex-col rounded-2xl border border-brand-rim bg-white p-4 shadow-sm shadow-brand-base/5 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-brand-gold hover:bg-brand-gold/10 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold"
            href={href}
            key={label}
          >
            ...
            <span className="mt-5 flex min-w-0 items-center justify-between gap-2 text-lg font-semibold text-slate-900">
              ...
              <ArrowRight className="size-4 shrink-0 text-slate-400 transition group-hover:translate-x-1 group-hover:text-brand-gold" aria-hidden="true" />
            </span>
