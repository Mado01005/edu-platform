...
export default function PromotionModal({ open, userEmail }: PromotionModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 fade-in">
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-emerald-950/10 bg-white p-6 shadow-xl custom-scrollbar scale-in sm:p-12">
        <div className="absolute inset-x-0 top-0 z-20 h-1 bg-[#063A2F]"></div>

        <div className="relative z-10 text-center">
          <div className="mx-auto mb-8 flex h-24 w-24 items-center justify-center rounded-full border-4 border-emerald-100 bg-[#063A2F] text-5xl sm:h-28 sm:w-28 sm:text-6xl">
            🎓
          </div>
          ...
          <p className="mb-4 text-base font-medium leading-relaxed text-slate-600 sm:text-lg">
            Congratulations, <span className="text-[#063A2F]">{userEmail}</span>! You have been officially promoted to an <strong className="text-slate-900">Instructor</strong>.
          </p>
          
          <div className="mb-10 rounded-2xl border border-emerald-950/10 bg-[#F4F7F4] p-6 text-left">
          ...
          <button
            ...
            disabled={loading}
            className="mx-auto flex w-full items-center justify-center gap-3 rounded-xl bg-[#063A2F] px-10 py-4 font-semibold text-white transition hover:bg-[#0A4235] disabled:opacity-50 sm:w-auto"
          >
