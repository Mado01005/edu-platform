              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {folders.map((folder, idx) => (
                  <motion.button
                    ...
                    className="group flex items-center gap-4 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-emerald-500/30 rounded-2xl p-4 transition-all duration-300 hover:shadow-[0_10px_30px_-10px_rgba(5,47,38,0.2)] text-left"
                    ...
                    <motion.div variants={itemVariants} key={`docview-${uniqueKey}`} className="flex flex-col gap-4 relative group">
                      <div className="flex items-center justify-between ml-1">
                        <div className="flex items-center gap-3">
                          <span className={`w-8 h-8 rounded-lg ${isPPT ? 'bg-orange-500/20 border-orange-500/30' : 'bg-[#052F26]/20 border-[#052F26]/30'} border flex items-center justify-center text-sm shadow-inner`}>
                            {isPPT ? '📊' : '📝'}
                          </span>
