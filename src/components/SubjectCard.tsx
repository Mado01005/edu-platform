...
export default function SubjectCard({ slug, title, icon, lessonCount, completedCount }: SubjectCardProps) {
  return (
    <Link
      ...
      id={`subject-${slug}`}
      className="group relative block cursor-pointer overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-1 hover:border-sky-300 hover:shadow-md md:p-8"
    >
      <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-sky-50" />

      <div className="relative z-10 mb-5 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-sky-100 text-3xl transition-transform duration-200 group-hover:scale-105">
        {icon}
      </div>

      <h2 className="relative z-10 mb-1.5 text-xl font-semibold text-slate-900 transition-colors group-hover:text-sky-700">
        {title}
      </h2>
      ...
      {/* Action Prompt */}
      <div className={`relative z-10 flex items-center gap-2 ${completedCount !== undefined ? 'mt-5' : 'mt-6'} text-sm font-semibold text-sky-700 transition`}>
