    ...
    <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
      {achievements.map((ach) => (
        ...
        >
          ...
          <h4 className="mb-2 break-words text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-900 transition-colors group-hover:text-[#052F26]">{ach.name}</h4>
          ...
          {ach.isUnlocked && (
             <div className="mt-4 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1">
               <span className="text-[8px] font-semibold uppercase tracking-wider text-[#052F26]">Achieved</span>
             </div>
          )}
        </div>
      ))}
