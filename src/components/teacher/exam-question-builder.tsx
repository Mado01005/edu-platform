...
export function ExamQuestionBuilder({
  ...
  return (
    <section className="border-t border-emerald-950/10 bg-[#F4F7F4] p-3">
      ...
      <button className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#063A2F] px-4 text-sm font-black text-white disabled:opacity-50" disabled={pending || !questions.length} onClick={() => void save()} type="button">{pending ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />} Save question bank</button>
