...
function BasicDetails({ course }: { course: TeacherCourse }) {
  ...
  return (
    <form action={action} className="mx-auto flex w-full max-w-md min-w-0 flex-col gap-4 rounded-2xl border border-emerald-950/10 bg-white p-4 shadow-sm">
      ...
      <label className="text-sm font-bold text-slate-700">Course Title<input className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#084B2B] focus:ring-2 focus:ring-emerald-100" defaultValue={course.title} name="title" required /></label>
      <label className="text-sm font-bold text-slate-700">Description<textarea className="mt-2 min-h-32 w-full min-w-0 resize-y rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#084B2B] focus:ring-2 focus:ring-emerald-100" defaultValue={course.description ?? ''} name="description" /></label>
      <label className="text-sm font-bold text-slate-700">Thumbnail URL<input className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#084B2B] focus:ring-2 focus:ring-emerald-100" defaultValue={course.imageUrl ?? ''} name="imageUrl" placeholder="Cloudflare R2 image URL" type="url" /></label>
      <div className="grid min-w-0 grid-cols-1 gap-3">
        <label className="min-w-0 text-sm font-bold text-slate-700">Grade Level<select className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#084B2B]" defaultValue={course.gradeLevel ?? ''} name="gradeLevel"><option value="">All grades</option>{grades.map((grade, index) => <option key={grade} value={grade}>Grade {index + 1}</option>)}</select></label>
      </div>
      ...
      <ActionSubmitButton className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#084B2B] px-4 py-3 font-bold text-white hover:bg-[#063B22]" pendingLabel="Saving course…"><Save className="size-4" /> Save Basic Details</ActionSubmitButton>
    </form>
  );
}
  ...
  return (
    ...
    >
      <Tabs className="min-h-full w-full min-w-0 gap-0" defaultValue={initialTab}>
        <div
          className="sticky top-0 z-30 min-w-0 border-b border-emerald-950/10 bg-[#F8FAF7] pb-4"
