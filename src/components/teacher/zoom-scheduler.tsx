  ...
  return (
    <section className="flex w-full min-w-0 flex-col gap-4">
      <form action={action} className="mx-auto flex w-full max-w-md min-w-0 flex-col gap-4 rounded-2xl border border-emerald-950/10 bg-white p-4 shadow-sm">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-[#063A2F]">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-14 shrink-0 items-center justify-center rounded-2xl bg-[#063A2F] text-white shadow-sm"><Video className="size-7" /></span>
            <span className="min-w-0"><span className="block text-lg font-black">Schedule New Live Zoom Class</span><span className="mt-1 flex items-center gap-1 text-xs font-bold text-[#063A2F]"><CalendarDays className="size-3.5" /> Cairo · EET/EEST timezone</span></span>
          </div>
        </div>
        <label className="min-w-0 text-sm font-bold text-slate-700">Session Title
          <input className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#063A2F] focus:ring-2 focus:ring-emerald-100" name="title" placeholder="Weekly Mathematics review" required />
        </label>
        <label className="min-w-0 text-sm font-bold text-slate-700">Zoom Meeting URL
          <input className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none invalid:border-red-400 focus:border-[#063A2F] focus:ring-2 focus:ring-emerald-100" name="meetingUrl" pattern="https://.*zoom\.us/j/.+" placeholder="https://zoom.us/j/123456789" required type="url" />
        ...
        <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
          <label className="min-w-0 text-sm font-bold text-slate-700">Start Date & Time
            <input className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#063A2F] focus:ring-2 focus:ring-emerald-100" name="startTime" onChange={(event) => setStartTime(event.target.value)} required type="datetime-local" value={startTime} />
            <span className="mt-1 block text-xs font-normal text-[#063A2F]">Cairo local time (EET/EEST)</span>
          ...
          <label className="min-w-0 text-sm font-bold text-slate-700">Duration (Minutes)
            <input className="mt-2 w-full min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-3 text-slate-900 outline-none focus:border-[#063A2F] focus:ring-2 focus:ring-emerald-100" defaultValue="60" max="480" min="5" name="duration" required type="number" />
          </label>
        </div>
        ...
        <ActionSubmitButton className="flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-[#063A2F] px-4 py-3 text-base font-black text-white shadow-sm hover:bg-[#0A4235]" pendingLabel="Scheduling…"><CalendarPlus className="size-5" /> Schedule New Live Zoom Class</ActionSubmitButton>
      </form>
      ...
      <section className="min-w-0 rounded-2xl border border-emerald-950/10 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 flex min-w-0 flex-col gap-3">
          {course.zoomSessions.map((session) => (
            <article className="flex min-w-0 flex-col gap-3 rounded-xl border border-emerald-950/10 bg-[#F4F7F4] p-3 sm:flex-row sm:items-center" key={session.id}>
