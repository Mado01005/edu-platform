    ...
    <section className="py-20 md:py-28" id="faculty">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-3 text-3xl font-black tracking-tight text-brand-white sm:text-5xl">
        ...
        <div className="mt-10 grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">
          {faculty.map((teacher) => (
            <article className="landing-card min-w-0 rounded-3xl border border-brand-border bg-brand-surface p-5 hover:-translate-y-1 hover:border-brand-gold/45 hover:shadow-lg" key={teacher.id}>
              <div className="flex min-w-0 items-center gap-4">
                {teacher.avatarUrl ? (
                  <Image alt={teacher.name} className="size-14 rounded-2xl object-cover" height={56} src={teacher.avatarUrl} width={56} />
                ) : (
                  <span className="flex size-14 shrink-0 items-center justify-center rounded-2xl bg-brand-base text-lg font-black text-brand-gold">{teacher.name.slice(0, 1).toUpperCase()}</span>
                )}
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {teacher.subjects.map((subject) => <span className="rounded-full border border-brand-gold/40 bg-brand-base px-2.5 py-1 text-[10px] font-black text-brand-gold" key={subject}>{subject}</span>)}
              </div>
              <p className="mt-5 flex items-start gap-2 border-t border-brand-border pt-4 text-xs leading-5 text-brand-muted/75">
                <MessageSquareQuote aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-brand-gold" />
