      ...
      <div className="mx-auto w-full max-w-7xl rounded-3xl border border-brand-rim bg-brand-surface p-5 text-brand-white shadow-2xl shadow-black/30 backdrop-blur-md sm:p-7">
        ...
        <div className="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-brand-border md:grid-cols-3 lg:grid-cols-6">
          {landingContent.trust.items.map((item, index) => {
            ...
            return (
              <div className="flex min-h-28 min-w-0 flex-col items-center justify-center gap-3 bg-brand-base px-3 py-4 text-center backdrop-blur-md" key={item.en}>
                <Icon aria-hidden="true" className="size-5 text-brand-gold" />
                <LandingCopy className="text-[11px] font-extrabold leading-5 text-brand-muted">{item}</LandingCopy>
              </div>
            );
          })}
        </div>
