      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {landingContent.problem.items.map((item, index) => {
            ...
            return (
              <article className="landing-card min-w-0 rounded-3xl border border-brand-mint-border bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-brand-gold/45 hover:shadow-md sm:p-8" key={item.title.en}>
                <div className="flex items-center justify-between gap-4"><span className="flex size-12 items-center justify-center rounded-full bg-brand-sage text-brand-base"><Icon aria-hidden="true" className="size-5" /></span><span aria-hidden="true" className="flex size-10 items-center justify-center rounded-full bg-brand-gold text-sm font-black text-brand-base">0{index + 1}</span></div>
        ...
        <div className="mt-6 rounded-3xl border border-brand-base/10 border-s-4 border-s-brand-gold bg-white px-6 py-6 text-brand-base shadow-sm sm:px-8"><LandingCopy className="text-base font-black leading-8 sm:text-lg">{landingContent.problem.bridge}</LandingCopy></div>
      </div>
    </section>
  );
}
