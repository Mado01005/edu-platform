...
export function ProblemSection() {
  return (
    <section className="scroll-mt-28 py-20 sm:py-28" id="programs">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {landingContent.problem.items.map((item, index) => {
            ...
            return (
              <article className="landing-card min-w-0 rounded-3xl border border-emerald-950/10 bg-white p-6 hover:-translate-y-0.5 hover:border-emerald-900/20 sm:p-8" key={item.title.en}>
        ...
        <div className="mt-6 rounded-3xl border-s-4 border-[#D4AF37] bg-[#084B2B] px-6 py-6 text-white sm:px-8"><LandingCopy className="text-base font-black leading-8 sm:text-lg">{landingContent.problem.bridge}</LandingCopy></div>
      </div>
    </section>
  );
}
