...
export function SolutionFramework() {
  return (
    <section className="scroll-mt-28 bg-[#042D1A] py-20 text-white md:py-28" id="how-it-works">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-14">
          <div className="lg:sticky lg:top-32 lg:self-start">
            ...
            <LandingCopy as="p" className="mt-8 rounded-2xl border border-emerald-500/15 bg-emerald-950/50 p-5 text-sm font-black leading-7 text-[#F4E8B7] shadow-2xl shadow-black/30 backdrop-blur-md">{landingContent.solution.conclusion}</LandingCopy>
          </div>
          <ol className="grid gap-3 sm:grid-cols-2">
            {landingContent.solution.steps.map(([title, description], index) => {
              const Icon = icons[index];
              return (
                <li className="landing-card group rounded-3xl border border-emerald-500/15 bg-emerald-950/50 p-5 shadow-2xl shadow-black/30 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-[#D4AF37]/35 hover:bg-emerald-950/65 sm:p-6" key={title.en}>
