...
const subjects = ['Physics', 'Pure Mathematics', 'Mechanics', 'Chemistry', 'Biology'] as const;
...
export function SubjectExplorer({ courses }: { courses: LandingCourse[] }) {
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-[#0F6E41]">
              <span data-language-copy="en">08 / Curriculum explorer</span>
              <span data-language-copy="ar">08 / مستكشف المناهج</span>
            </p>
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">
              Browse the published path first. Course pricing remains inside the catalog and authenticated checkout.
            </p>
          </div>
          ...
          <div aria-label="Curriculum filter type" className="inline-flex w-fit rounded-full bg-slate-100 p-1" role="tablist">
            {(['grade', 'subject'] as const).map((value) => (
              <button
                ...
                className={`min-h-11 rounded-full px-4 text-xs font-extrabold transition-all ${mode === value ? 'bg-white text-[#084B2B] shadow-sm' : 'text-slate-500 hover:text-[#084B2B]'}`}
                key={value}
            ...
            {(mode === 'grade' ? grades : subjects).map((value) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-11 shrink-0 rounded-full px-4 text-xs font-extrabold transition-all ${selected ? 'bg-[#084B2B] text-white shadow-sm' : 'text-slate-500 hover:bg-white hover:text-[#084B2B]'}`}
                  key={value}
                  onClick={() => {
        ...
        <div className="mt-6 grid min-w-0 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((course) => {
            ...
            const previewHref = `/catalog?course=${encodeURIComponent(course.id)}&preview=lesson-1`;

            return (
              <article className="landing-card group flex min-w-0 flex-col overflow-hidden rounded-3xl border border-emerald-950/5 bg-white p-5 hover:-translate-y-1 hover:shadow-lg" key={course.id}>
