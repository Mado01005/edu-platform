...
export function OutcomesSection() {
  return (
    <section className="scroll-mt-28 bg-brand-ivory py-20 text-brand-base md:py-28" id="outcomes">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <LandingCopy className="text-xs font-black uppercase tracking-[0.18em] text-brand-base underline decoration-brand-gold decoration-2 underline-offset-4">{landingContent.outcomes.eyebrow}</LandingCopy>
          <LandingCopy as="h2" className="mt-4 text-balance text-3xl font-black tracking-tight text-brand-base sm:text-5xl">{landingContent.outcomes.title}</LandingCopy>
          <LandingCopy as="p" className="mt-5 text-sm leading-7 text-brand-surface/80 sm:text-base">{landingContent.outcomes.description}</LandingCopy>
        </div>
            ...
            <ol className="mt-8 grid gap-3 md:grid-cols-5">
              {landingContent.outcomes.sequence.map((item, index) => (
                <li className="landing-card relative flex min-h-28 flex-col justify-between rounded-2xl border border-brand-base/10 bg-brand-ivory-alt p-4 shadow-lg shadow-brand-base/5 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-brand-gold/45" key={item.en}>
                  ...
                  <LandingCopy className="mt-4 text-sm font-black leading-6 text-brand-base">{item}</LandingCopy>
