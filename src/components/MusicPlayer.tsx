...
const MusicPlayer = () => {
  ...
  /* ── Minimized State: Sleek Spotify icon, dead-center bottom ── */
  if (isMinimized) {
    return (
      <button
      ...
      >
        ...
        {isPlaying && (
          <span className="absolute inset-0 rounded-full border-2 border-[#1DB954]/50 animate-ping" />
        )}
        {/* Mini album art ring when connected */}
  ...
  /* ── Expanded State: Center-bottom card, mobile-responsive ── */
  return (
    <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-[90vw] md:w-[400px] z-[99999] transition-all duration-300">
      <div className="relative">
        {/* Glass Container */}
        <div className="bg-[#0f0f13]/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.8)] overflow-hidden">
          
          {/* Minimize Button (Chevron Down) */}
          <button
            onClick={() => setIsMinimized(true)}
            className="absolute top-3 right-3 z-30 w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all"
            title="Minimize"
          >
          ...
          {(!hasToken || isTokenExpired) ? (
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex items-center gap-4">
                <div className="relative w-16 h-16 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10 flex-shrink-0">
              ...
              {/* Playlist Dropdown */}
              {showPlaylists && (
                <div className="max-h-48 overflow-y-auto bg-white/[0.03] backdrop-blur-sm p-4 space-y-2 rounded-xl border border-white/5">
                  ...
                  <div className="grid gap-2">
                    {playlists.map((pl: any) => (
                      <button key={pl.id} onClick={() => playPlaylist(pl.uri)} className="flex items-center gap-3 p-2 rounded-xl bg-white/5 hover:bg-white/10 transition-all text-left">
