...
export function PasswordlessParentLogin({ enabled }: { enabled: boolean }) {
  ...
  if (verifiedPhone) {
    return <form className="flex min-w-0 flex-col gap-3" onSubmit={verifyCode}><label className="text-xs font-bold text-slate-600">6-digit SMS code<input autoComplete="one-time-code" autoFocus className="mt-2 h-14 w-full rounded-xl border border-emerald-950/10 bg-white text-center text-2xl font-black tracking-[0.35em] outline-none focus:border-[#063A2F] focus:ring-4 focus:ring-emerald-100" inputMode="numeric" maxLength={6} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} value={code} /></label>{notice ? <p className="text-xs text-emerald-700">{notice}</p> : null}{error ? <p aria-live="assertive" className="text-xs font-bold text-red-700">{error}</p> : null}<button className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#063A2F] px-4 font-black text-white disabled:opacity-50" disabled={pending || code.length !== 6} type="submit">{pending ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Verify and open dashboard</button><button className="text-xs font-bold text-slate-500" disabled={pending} onClick={() => { setVerifiedPhone(''); setCode(''); setError(''); }} type="button">Use a different number</button></form>;
  }

  return <form className="flex min-w-0 flex-col gap-3" onSubmit={sendCode}><label className="text-xs font-bold text-slate-600">Parent mobile number<PhoneInput className="mt-2" disabled={pending} onChange={setPhone} required value={phone} /></label><p className="flex items-start gap-2 rounded-xl border border-[#D8A84E]/40 bg-[#F4F7F4] p-3 text-xs leading-5 text-[#805C1D]"><KeyRound className="mt-0.5 size-4 shrink-0" /> Use the exact number saved in the student&apos;s onboarding profile.</p>{error ? <p aria-live="assertive" className="text-xs font-bold text-red-700">{error}</p> : null}<button className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#063A2F] px-4 font-black text-white disabled:opacity-50" disabled={pending || !phone} type="submit">{pending ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />} Send passwordless code</button></form>;
}
