...
export function ExamRunner({
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-[#D8A649]/40 bg-white p-4 shadow-sm sm:p-5">
      <header className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
        ...
        {attempt && !result ? <span className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 font-mono text-sm font-black ${remaining < 60_000 ? 'bg-red-100 text-red-700' : 'bg-[#EEF5F1] text-[#805C1D]'}`}><Clock3 className="size-4" /> {timeLabel(remaining)}</span> : null}
      </header>

      {!attempt ? (
        <div className="mt-4 rounded-xl border border-emerald-950/10 bg-[#EEF5F1] p-4 text-sm text-slate-600">
          ...
          <button className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#052F26] px-4 font-black text-white hover:bg-[#093F33] disabled:opacity-50" disabled={pending || attemptsUsed >= maxAttempts} onClick={() => void start()} type="button">{pending ? <Loader2 className="size-4 animate-spin" /> : <Clock3 className="size-4" />} Start exam</button>
        </div>
      ) : null}
      ...
      {attempt && !result ? (
        <div className="mt-5 flex min-w-0 flex-col gap-4">
          {attempt.questions.map((question, questionIndex) => (
            <article className="min-w-0 rounded-2xl border border-emerald-950/10 bg-[#EEF5F1] p-4" key={question.id}>
              <p className="text-xs font-black uppercase tracking-wide text-[#052F26]">Question {questionIndex + 1} of {attempt.questions.length}</p>
              ...
              <fieldset className="mt-4 flex min-w-0 flex-col gap-2">
                ...
                {question.options.map((option) => (
                  <label className={`flex min-w-0 cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm transition ${answers[question.id] === option.key ? 'border-[#D8A649] bg-[#EEF5F1]' : 'border-emerald-950/10 bg-white hover:border-emerald-300'}`} key={option.key}>
          ...
          <button className="sticky bottom-20 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#052F26] px-4 font-black text-white shadow-lg hover:bg-[#093F33] disabled:opacity-50 md:bottom-4" disabled={pending} onClick={() => void submit()} type="button">{pending ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />} Submit exam · {answeredCount}/{attempt.questions.length} answered</button>
        </div>
      ) : null}
        ...
        <div className="mt-5">
          ...
          <div className="mt-4 flex min-w-0 flex-col gap-3">
            {result.review.map((question, index) => (
              <article className="rounded-xl border border-emerald-950/10 bg-[#EEF5F1] p-4" key={question.id}>
                <p className="text-xs font-black text-[#052F26]">Question {index + 1}</p>
                ...
                <div className="mt-3 rounded-xl border border-[#D8A649]/40 bg-[#EEF5F1] p-3"><p className="text-xs font-black uppercase tracking-wide text-[#805C1D]">Worked solution</p><MathText className="mt-2 block whitespace-pre-wrap text-sm leading-6 text-slate-700" value={question.workedSolution} /></div>
              </article>
            ))}
          </div>
          {remainingAttempts > 0 ? <button className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#052F26] px-4 font-black text-[#052F26] hover:bg-emerald-50" onClick={() => { setAttempt(null); setResult(null); }} type="button"><RotateCcw className="size-4" /> Start retake · {remainingAttempts} remaining</button> : null}
        </div>
      ) : null}
